...
function send(text){
 ...
 // Solo el emisor genera la respuesta, junto a su pregunta, en una escritura atómica.
 if(!user.gm&&(currentChannel==='mecenas'||(currentChannel==='general'&&(/@mecenas/i.test(msg)||/[?¿]/.test(msg))))){
  const entry=answerQuestion(msg);patch['messages/reply_'+id]={id:'reply_'+id,msg:entry.answer,who:'Mecenas',userId:'mecenas',ch:currentChannel,ts:ts+1,t:'text',bot:true,topic:entry.id,replyTo:id,requester:user.id,...(entry.check?{check:entry.check.skill}:{})};
 }
...
function renderFindings(data){const el=$('#findings'),latest=new Map();for(const m of messages(data))if(m.bot&&['general','mecenas'].includes(m.ch)&&!['fallback','saludo','limite','mesa'].includes(m.topic))latest.set(m.topic,m);
...
$('#exportPlan').onclick=()=>{
 ...
 lines.push('## Información reunida');for(const m of messages())if(m.bot&&['mecenas','general'].includes(m.ch))lines.push('- '+m.msg);
...
function roll(label,val){
 ...
 if(pending){const entry=FAQ.find(e=>e.id===pending.topic),reply=resolveCheck(entry,record);if(reply)patch['messages/resolved_'+pending.id]={id:'resolved_'+pending.id,msg:reply.answer,who:'Mecenas',userId:'mecenas',bot:true,topic:reply.id,ch:channel,ts:ts+1,resolvedFor:pending.id};}
